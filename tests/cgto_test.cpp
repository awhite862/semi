/** \brief A test class for CGTO overlaps.*/
...
int run_cgto_test() {
    ...
    ar << 1 << 0.01 << 0.001;
    QNumber aq(1, 0, 0);
    GTOFunction a(aq, 0, 0, 0, 10, ar);

    arma::colvec br;
    br << -1 << -0.01 << -0.001;
    QNumber bq(1, 0, 0);
    GTOFunction b(bq, 0, 0, 0, 10, br);
    ...
    ar2 << 1 << 0.01 << 0.001;
    QNumber aq2(1, 1, 0);
    GTOFunction a2(aq2, 1, 0, 0, 10, ar2);

    arma::colvec br2;
    br2 << -1 << -0.01 << -0.001;
    QNumber bq2(1, 1, 0);
    GTOFunction b2(bq2, 1, 0, 0, 10, br2);
    ...
    ar3 << 10 << 1 << 0.1;
    QNumber aq3(1, 1, 0);
    GTOFunction a3(aq3, 1, 0, 0, 0.01, ar3);

    arma::colvec br3;
    br3 << -10 << -1 << -0.1;
    QNumber bq3(1, 1, 0);
    GTOFunction b3(bq3, 1, 0, 0, 0.05, br3);
...
int run_cgto_matrix_test() {
    ...
    ar << 1 << 0.01 << 0.001;
    QNumber aq(1, 0, 0);
    GTOFunction a(aq, 0, 0, 0, 3.4252509, ar);

    arma::colvec br;
    br << 1 << 0.01 << 0.001;
    QNumber bq(1, 0, 0);
    GTOFunction b(bq, 0, 0, 0, 0.6239137, br);

    arma::colvec cr;
    cr << 1 << 0.01 << 0.001;
    QNumber cq(1, 0, 0);
    GTOFunction c(cq, 0, 0, 0, 0.1688554, cr);

    arma::colvec dr;
    dr << 2 << 0.01 << 0.001;
    QNumber dq(1, 0, 0);
    GTOFunction d(dq, 0, 0, 0, 3.4252509, dr);

    arma::colvec er;
    er << 2 << 0.01 << 0.001;
    QNumber eq(1, 0, 0);
    GTOFunction e(eq, 0, 0, 0, 0.6239137, er);

    arma::colvec fr;
    fr << 2 << 0.01 << 0.001;
    QNumber fq(1, 0, 0);
    GTOFunction f(fq, 0, 0, 0, 0.1688554, fr);

    CGTOFunction abc = CGTOFunction(aq, 0, 0, 0, ar, 1);

    CGTOFunction def = CGTOFunction(dq, 0, 0, 0, dr, 1);
